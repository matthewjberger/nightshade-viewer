...
fn fragment_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grid_color_thin = vec4<f32>(0.75, 0.75, 0.75, 0.4);
    let grid_color_thick = vec4<f32>(0.2, 0.4, 0.8, 0.6);

    var color: vec4<f32>;
    if (lod2_alpha > 0.0) {
        color = grid_color_thick;
        color.a *= lod2_alpha * 1.0;
    } else if (lod1_alpha > 0.0) {
        let fade = smoothstep(0.2, 0.8, lod_fade);
        color = mix(grid_color_thick, grid_color_thin, fade);
        color.a *= lod1_alpha * 0.8;
    } else {
        color = grid_color_thin;
        color.a *= (lod0_alpha * (1.0 - lod_fade)) * 0.7;
    }
    ...
    // Kept the discard threshold the same to maintain edge quality
    if (color.a < 0.02) {
        discard;
    }
